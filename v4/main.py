...
def method_2(x,y):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
                             x, y, test_size=0.33, random_state=42)
    def cost_mse(list):
        mse = ((((list[0]+(1.0355**(-X_train+list[1])))-y_train)**2)).sum()/len(y_train)
        return mse 
    pkonst=minimize(cost_mse,x0=np.array([4,120]),method='Powell')
    end=time.time()
    mse2=mse((pkonst.x[0]+(1.0355**(-X_test+pkonst.x[1]))),y_test)
    mape2=mape((pkonst.x[0]+(1.0355*(-X_test+pkonst.x[1]))),y_test)
...
def method_3(x,y):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
                             x, y, test_size=0.33, random_state=42)
    popt, pcov = curve_fit(func, X_train, y_train,p0=[65,1.0355,4],bounds=([0,1,0], [670, 1.15, 30]), maxfev=5000)
    end=time.time()
...
def method_4(x,y):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
                             x, y, test_size=0.33, random_state=42)
    popt, pcov = curve_fit(func, X_train, y_train,p0=[120,4])
    end=time.time()
    mse3=mse(func(X_test, *popt),y_test)
    mape3=mape(func(X_test, *popt),y_test)
    return mse3,mape3,(end-start)
...
if __name__ == '__main__':
    ...
    p2=Process(target=process, args=(method_2,"Curve_Fit_2D",df,q,))
    p3=Process(target=process, args=(method_3,"Curve_Fit_3D",df,q,)) 
    p4=Process(target=process, args=(method_4,"Powell_2D",df,q,))
    p1.start()
    p2.start()
    p3.start()
    p4.start()
