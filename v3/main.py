...
def method_2(x,y):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
                             x, y, test_size=0.33, random_state=42)
    def cost_mse(list):
        mse = ((((list[0]+(list[1]**(-X_train)))-y_train)**2)).sum()/len(y_train)
        return mse 
    pkonst=minimize(cost_mse,x0=np.array([10,np.exp(1)]),method='Powell')
    end=time.time()
    mse2=mse((pkonst.x[0]+(pkonst.x[1]**(-X_test))),y_test)
    mape2=mape((pkonst.x[0]+(pkonst.x[1]**(-X_test))),y_test)
...
def method_4(x,y):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
                             x, y, test_size=0.33, random_state=42)
    popt, pcov = curve_fit(func, X_train, y_train,p0=[5,np.exp(1)])
    end=time.time()
    mse3=mse(func(X_test, *popt),y_test)
    mape3=mape(func(X_test, *popt),y_test)
    return mse3,mape3,(end-start)
